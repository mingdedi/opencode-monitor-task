...
| `storm-sim.sh` | Simulated service error storm: healthy start, then a 30-line `ERROR` burst and a `FATAL`, exit 1. Run it twice — with the default `coalesce_ms` (burst merges into one notification) and with `coalesce_ms=0` (merging off; token bucket drops all but 5). Suggested pattern: `ERROR|FATAL` |
